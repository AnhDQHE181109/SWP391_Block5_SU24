...
public class SearchingController extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        if (query != null && !query.trim().isEmpty()) {
            String sql = "SELECT p.ProductID, p.ProductName, pi.ImageURL FROM Products p LEFT JOIN ProductImages pi ON p.ImageID = pi.ImageID WHERE p.ProductName LIKE ?";
            try (PreparedStatement ps = conn.prepareStatement(sql)) {
                ...
                while (rs.next()) {
                    ...
                    // Construct an HTML snippet for each suggestion with a clickable link
                    String suggestionHTML = "<div class='list-group-item' data-url='" + request.getContextPath() + "/ProductDetailsController?productID=" + productId + "'>" +
